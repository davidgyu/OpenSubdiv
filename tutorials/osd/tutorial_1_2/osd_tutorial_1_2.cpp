...
//  --no-normals       : don't compute normals.
//  --fvar             : compute face varying values.
//  --refinelevel <int>: topological refinement level.
...
createDefaultTopologyRefiner(
    ...
    *modelName = "defaultCube";
    modelPoints->assign(&g_verts[0], &g_verts[3*g_nverts]);
    modelUVs->assign(&g_uvs[0], &g_uvs[2*g_nuvs]);
...
createTopologyRefiner(
    ...
    *modelName = filepath;
    modelPoints->assign(shape->verts.begin(), shape->verts.end());
    modelUVs->assign(shape->uvs.begin(), shape->uvs.end());
...
Tessellator::Tessellator(
    ...
    // XXX refiner is unused, but could help eliminate duplicated points.
...
refineMeshAdaptive(
    ...
    std::vector<float> const & modelPoints,
    std::vector<float> const & modelUVs,
    Far::TopologyRefiner * refiner,
    int refinelevel, int tesslevel,
    bool computeNormals, int fvarChannel)
{
    // FaceVarying topology for UV data
    bool computeFVar = (fvarChannel >= 0) && (refiner->GetNumFVarChannels() > 0);
    ...
    if (computeFVar) {
        ...
        patchOptions.numFVarChannels = refiner->GetNumFVarChannels();
    ...
    if (computeFVar) {
        ...
        options.fvarChannel = fvarChannel;
        options.generateOffsets = true;
        options.generateIntermediateLevels = true;
    ...
    int nBaseUVs = 0;
    if (computeFVar) {
        nBaseUVs = refiner->GetLevel(0).GetNumFVarValues(fvarChannel);
    }
    ...
    pointsBuffer->UpdateData(&modelPoints[0], 0, nBasePoints);
    ...
    if (computeFVar) {
        ...
        uvBuffer->UpdateData(&modelUVs[0], 0, nBaseUVs);
    ...
    printf("# %s adaptive tessellation refinelevel=%d tesslevel=%d\n",
           modelName.c_str(), refinelevel, tesslevel);
...
refineMeshUniform(
    ...
    std::vector<float> const & modelUVs,
    Far::TopologyRefiner * refiner,
    int refinelevel, bool computeNormals, int fvarChannel)
{
    //
    // 1) Refine the mesh topology
    //
    ...
    Far::TopologyRefiner::UniformOptions uniformOptions(refinelevel);
    ...
    Far::TopologyLevel const & lastLevel = refiner->GetLevel(refinelevel);
    ...
    std::copy((PrimvarPoints*)&modelPoints[0],
              (PrimvarPoints*)&modelPoints[3*nBasePoints],
              pointsBuffer.begin());
    ...
    for (int level=0; level < refinelevel; ++level) {
        PrimvarPoints * const dstPoints =
            srcPoints + refiner->GetLevel(level).GetNumVertices();

        primvarRefiner.Interpolate(level+1, srcPoints, dstPoints);
    ...
    if (computeFVar) {
        ...
        uvBuffer.resize(refiner->GetNumFVarValuesTotal(fvarChannel));
        std::copy((PrimvarUVs*)&modelUVs[0],
                  (PrimvarUVs*)&modelUVs[2*nBaseUVs],
                  uvBuffer.begin());
        ...
        for (int level=0; level < refinelevel; ++level) {
            PrimvarUVs * const dstUVs =
                srcUVs + refiner->GetLevel(level).GetNumFVarValues(fvarChannel);

            primvarRefiner.InterpolateFaceVarying(level+1, srcUVs, dstUVs,
                                                  fvarChannel);
        ...
        int nLimitUVs = lastLevel.GetNumFVarValues(fvarChannel);
        limitUVs.resize(nLimitUVs);

        // Evaluate limit position UVs
        primvarRefiner.LimitFaceVarying(lastRefinedUVs, limitUVs, fvarChannel);
    }
    ...
    printf("# %s uniform refinement refinelevel=%d\n",
           modelName.c_str(), refinelevel);
    ...
    for (int face=0, fv=0; face<lastLevel.GetNumFaces(); ++face) {
        ...
        if (computeFVar) {
            Far::ConstIndexArray faceFVarValues =
                    lastLevel.GetFaceFVarValues(face, fvarChannel);
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        if (!strcmp(argv[i], "--adaptive")) {
            ...
            computeNormals = true;
        } else if (!strcmp(argv[i], "--no-normals")) {
            computeNormals = false;
        } else if (!strcmp(argv[i], "--refinelevel") && (i+1) < argc) {
            refinelevel = atoi(argv[++i]);
            ...
            printf("  --no-normals       : don't compute normals.\n");
            printf("  --fvar             : compute face varying values.\n");
            printf("  --refinelevel <int>: topological refinement level.\n");
    ...
    // Load the model and create a topology refiner
    std::string modelName;
    std::vector<float> modelPoints;
    std::vector<float> modelUVs;

    Far::TopologyRefiner * refiner = filepath.empty()
        ? createDefaultTopologyRefiner(&modelName, &modelPoints, &modelUVs)
        : createTopologyRefiner(filepath, &modelName, &modelPoints, &modelUVs);

    // Refine and output the tessellated result
    if (refineAdaptive) {
        refineMeshAdaptive(modelName, modelPoints, modelUVs,
                refiner, refinelevel, tesslevel, computeNormals, fvarChannel);
    } else {
        refineMeshUniform(modelName, modelPoints, modelUVs,
                refiner, refinelevel, computeNormals, fvarChannel);
    }
