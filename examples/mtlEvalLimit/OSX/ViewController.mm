...
-(void)viewDidLoad {
    ...
    auto callbackCheckbox = [=](bool value, int ID) {
        switch(ID) {
            ...
            case kHUD_CB_RANDOM_START:
                self.osdRenderer.useRandomStart = value;
                break;
            case kHUD_CB_ANIMATE_PARTICLES:
                self.osdRenderer.useAnimateParticles = value;
                break;
            default:
    ...
    auto callbackShadingMode = [=](int shadingMode) {
        switch((ShadingMode)shadingMode) {
            case kShadingUV:
            case kShadingVARYING:
            case kShadingNORMAL:
            case kShadingSHADE:
            case kShadingFACEVARYING:
            case kShadingMEAN_CURVATURE:
    ...
    y += 20;
    hud.AddCheckBox("Random Start (T)",  _osdRenderer.useRandomStart,
                      10, y, callbackCheckbox, kHUD_CB_RANDOM_START, 't');
    y += 20;
    hud.AddCheckBox("Animate Particles (P)",  _osdRenderer.useAnimateParticles,
                      10, y, callbackCheckbox, kHUD_CB_ANIMATE_PARTICLES, 'p');
    y += 20;
    hud.AddCheckBox("Freeze (spc)", _osdRenderer.freeze,
                    10, y, callbackCheckbox, kHUD_CB_FREEZE, ' ');
    y += 20;
    
    int shading_pulldown = hud.AddPullDown("Shading (W)", 200, 10, 250,
                                                callbackShadingMode, 'w');
    hud.AddPullDownButton(shading_pulldown, "UV",
                            kShadingUV,
                            _osdRenderer.shadingMode == kShadingUV);
    hud.AddPullDownButton(shading_pulldown, "Varying",
                            kShadingVARYING,
                            _osdRenderer.shadingMode == kShadingVARYING);
    hud.AddPullDownButton(shading_pulldown, "Normal",
                            kShadingNORMAL,
                            _osdRenderer.shadingMode == kShadingNORMAL);
    hud.AddPullDownButton(shading_pulldown, "Shade",
                            kShadingSHADE,
                            _osdRenderer.shadingMode == kShadingSHADE);
    hud.AddPullDownButton(shading_pulldown, "Face Varying",
                          kShadingFACEVARYING,
                          _osdRenderer.shadingMode == kShadingFACEVARYING);
    hud.AddPullDownButton(shading_pulldown, "Mean Curvature",
                          kShadingMEAN_CURVATURE,
                          _osdRenderer.shadingMode == kShadingMEAN_CURVATURE);
...
-(void)drawInMTKView:(MTKView *)view {
    ...
    auto renderEncoder = [_osdRenderer drawFrame:commandBuffer
                                       frameBeginTimestamp:_frameBeginTimestamp[frameId]];
    auto& hud = self.view->hud;
    if(hud.IsVisible()) {
        if(_showPatchCounts) {
            int x = -280;
